...
export const blueprint: Blueprint = {
    ...
    test(plr, self) {
        ...
        assert(() => game.interact.card.doPlaceholders(self) === "Battlecry: Gain mana equal to the turn counter. (Currently 1, haha lol, 1, The next thing is: test, {placeholder without replacement})");

        game.endTurn();
        game.endTurn();

        assert(() => game.interact.card.doPlaceholders(self) === "Battlecry: Gain mana equal to the turn counter. (Currently 2, haha lol, 2, The next thing is: test, {placeholder without replacement})");
    }
}
