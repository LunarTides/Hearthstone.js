...
export const blueprint: Blueprint = {
	...
	collectible: false,
	tags: [],
	id: "019bc665-4f80-7024-ba8b-78e125a21f37",

	async heropower(self, owner) {
		// Filter away totem cards that is already on the player's side of the board.
		const filteredTotemCards = (await Card.allWithTags(Tag.BasicTotem)).filter(
	...
	async test(self, owner) {
		const totemCards = await Card.allWithTags(Tag.BasicTotem);
