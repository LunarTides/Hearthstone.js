...
This will first send the string to `processCommand`, which will check if the action is a valid command.
...
If this is successful, it will call `game.play` will which call this function as defined here:
    ```ts
    // src/modules/card/play.ts
...
It continues this until `game.running` becomes `false`. **Everything that happens in the game is a result of a command running, or a card being played.**
